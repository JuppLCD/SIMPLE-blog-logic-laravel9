    ...
    <form action="{{route('admin.posts.store')}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" placeholder="Leave a extract here" name='extract'
                id="extract">{{old('extract')}}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <div class="form-check">
                <input class="form-check-input" type="radio" name="status" id="status1" value="1" {{old('status', '1'
                    )==='1' ? 'checked' : '' }}>
            ...
            <div class="form-check">
                <input class="form-check-input" type="radio" name="status" id="status2" value="2" {{old('status')==='2'
                    ? 'checked' : '' }}>
        ...
        <div class="form-floating mb-5">
            <select class="form-select" id="category_id" aria-label="Floating label select example" name='category_id'>
                ...
                <option value="{{$id}}" {{old('category_id')===$id ? 'selected' : '' }}>{{$categoryName}}
                </option>
                @endforeach
            </select>
        ...
        <div class="mb-5">
            ...
            <label class="me-3">
                <input type="checkbox" name="tags[]" value="{{$id}}" {{in_array($id, old('tags',[] )) ? 'checked' : ''
                    }}>
                {{$tagName}}
            </label>
            @endforeach
